...
var frameTail = '?enablejsapi=1&autoplay=1&color=white&showinfo=0&rel=0&cc_load_policy=0&iv_load_policy=3&disablekb=1" frameborder="0" allowfullscreen></iframe>';
...
$(function getLinkAndAddSong () {
	$('#playlist > div > a').click(function () {
		...
		$(this).removeClass('songButtonStyle').css({backgroundColor: '#989293', color: 'white', fontWeight: 'bold'})
			.siblings().addClass('songButtonStyle');//.css({backgroundColor: 'transparent', color: 'initial', fontWeight: 'initial'});
...
$(function playNextOrPrevBtn() {
	$('#nextOption').click(function () {
		playNextSong();
	});
	$('#previousOption').click(function () {
...
function alertFade (text) {
	$('#alertKeyboard').html(text)
		.css({'font-size':'50px', 'opacity':'0.5','display':'block'})
		.animate({fontSize: '80px', opacity: '0'},
